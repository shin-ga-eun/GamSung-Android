...
public class HashSearchListViewAdapter extends BaseAdapter {

    // 버튼 클릭 이벤트를 위한 Listener 인터페이스 정의 -> 입장 버튼에 사용.
    public interface ListBtnClickListener {
        void onListBtnClick(String name) ;
    }
    private HashSearchListViewAdapter.ListBtnClickListener listBtnClickListener ;


    //Adapter에 추가된 데이터를 저장하기위한  ArrayList
    private ArrayList<HashSearchListViewItem> listViewItemList = new ArrayList<HashSearchListViewItem>();

    //InterestListViewAdapter생성자
    public HashSearchListViewAdapter(HashSearchListViewAdapter.ListBtnClickListener clickListener){
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if(convertView == null){
            ...
            convertView = inflater.inflate(R.layout.hashsearch_content_item, parent, false);
        }
        ...
        final HashSearchListViewItem listViewItem = listViewItemList.get(position);
    ...
    public void addItem(String hashname){
        HashSearchListViewItem item = new HashSearchListViewItem();
